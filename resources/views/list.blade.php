    ...
    <table border = "1" rules = "rows">
        <thead>
            <tr>
                ...
                <th>在庫数</th>
                <th>メーカー名</th>
                <th class="th7"><button class="create" type = "button" onclick = "location.href = '{{route('create')}}'">新規登録</button></th>
            </tr>
        </thead>
        <tbody>
        @foreach ($products as $product)
            <tr>
                ...
                <td><button class="info" type="button" onclick="location.href='{{route('info',['id' => $product ->id])}}'">詳細</button>
                    <button class="delete" type="button" onclick="deleteAlert({{ $product->id }})">削除</button>
                </td>
        @endforeach
        </tbody>
    </table>
    <script src="{{ asset('build/assets/delete.js') }}"></script>
</body>
</html>